'''
Програмима разделена на 3 части - A, B, C
part A включает в себя task5_1()
part B включает в себя task5_2(), task5_3(), task5_4()
part C включает в себя task5_5(), task5_6(), task5_7()
'''
...
def task5_7(text):
    ...
    # Регулярное выражение для захвата первых двух слов
    pattern = r'^(\(?\s?)(.*?)(,?\s?\)?)(\s+)(.*?)(\s+)(.*?)'
    result = r'\1\5\3\4\2\6\7'

    return re.sub(pattern,  result, text)
